...
ot.EditorClient = (function () {
	...
	function EditorClient (docId, revision, clients, serverAdapter, editorAdapter) {
		...
		this.serverAdapter.registerCallbacks({
			...
			ack: function (docId) {
				//console.log(self.docId+": Received ack for doc: "+docId);
				if(docId == self.docId){
					self.serverAck();
				}
			},
	...
	EditorClient.prototype.sendSelection = function (selection) {
		...
		this.serverAdapter.sendSelection(this.docId, selection);
	};
